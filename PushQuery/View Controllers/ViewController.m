...
#import "PQCoreDataController.h" // temp
#import "PQDataManager.h"
...
@property (nonatomic, strong) id <PQUser_Editable> user;
...
- (void)setUser:(id <PQUser_Editable>)user {
...
- (IBAction)signIn:(UIButton *)sender {
    ...
    if (self.user) {
        ...
        [PQLoginManager loginWithEmail:FirebaseAPITestEmail password:FirebaseAPITestPassword success:^(id <PQUser_Editable> currentUser) {
...
//    [PQDataManager isConnectedToFirebase:^(BOOL connected) {
//        if (connected) {
//            [PQDataManager disconnectFromFirebase];
//        }
//        else {
//            [PQDataManager connectToFirebase];
//        }
//    }];
    
    if ([PQDataManager isConnectedToFirebase]) {
        [PQDataManager disconnectFromFirebase];
    }
    else {
        [PQDataManager connectToFirebase];
...
- (void)addObserversToDataManager {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(firebaseConnectionDidChange:) name:PQFirebaseIsConnectedDidChangeNotification object:nil];
}

- (void)removeObserversFromDataManager {
    [AKDebugger logMethod:METHOD_NAME logType:AKLogTypeMethodName methodType:AKMethodTypeSetup tags:@[AKD_NOTIFICATION_CENTER] message:nil];
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:PQFirebaseIsConnectedDidChangeNotification object:nil];
}
