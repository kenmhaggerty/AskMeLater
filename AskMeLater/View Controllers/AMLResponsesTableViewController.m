//
//  AMLResponsesTableViewController.m
//  AskMeLater
//
...
#import "AMLResponsesTableViewController.h"
#import "AKDebugger.h"
#import "AKGenerics.h"
...
@property (nonatomic, strong) NSMutableOrderedSet <id <AMLResponse>> *responses;
@property (nonatomic, strong) NSComparisonResult (^sortComparator)(id <AMLResponse> response1, id <AMLResponse> response2);
...
@implementation AMLResponsesTableViewController
...
- (NSComparisonResult (^)(id <AMLResponse> response1, id <AMLResponse> response2))sortComparator {
    ...
    _sortComparator = ^NSComparisonResult(id <AMLResponse> response1, id <AMLResponse> response2) {
...
- (void)addObserversToQuestion:(id <AMLQuestion>)question {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(questionResponseWasAdded:) name:AMLQuestionResponseWasAddedNotification object:question];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(questionResponseWasRemoved:) name:AMLQuestionResponseWasRemovedNotification object:question];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(questionWillBeRemoved:) name:AMLQuestionWillBeRemovedNotification object:question];
}
...
- (void)removeObserversFromQuestion:(id <AMLQuestion>)question {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:AMLQuestionWillBeRemovedNotification object:question];
}
...
- (void)questionResponseWasAdded:(NSNotification *)notification {
    ...
    id <AMLResponse> response = notification.userInfo[NOTIFICATION_OBJECT_KEY];
